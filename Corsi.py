...
class CorsiBlockTappingTest():
	...
	def __init__(self):
		...
		self.win = visual.Window([1440, 1440], fullscr = False, pos = (0., 0.))
		self.mouse = event.Mouse(newPos= (0., 0.))
	...
	def setup(self):
		...
		self.thisExp.addData("No", self.expInfo["No"])
	...
	def simulate(self, n = 0):
		...
		for index in self.randseq:
			...
			self.win.flip()
			time.sleep(1.0)
			self.grid[index].fillColor = "black"
	...
	def getResult(self, n = 0):
		...
		while len(self.selected) < n:
			for i, shape in enumerate(self.grid):
				if self.mouse.getPressed()[0] & shape.contains(self.mouse.getPos() + 0):
	...
	def showAnswer(self):
		...
		if isCorrect:
			print("Correct")
			if self.KEY < self.num:
				self.KEY = self.num
				self.numLives -= 1
			elif self.KEY == self.num:
				self.num += 1
				self.numLives = self.LIVES
		else:
			print("Incorrect")
			self.numLives -= 1
			if self.numLives == 1 and self.KEY < self.num:
				print(self.KEY,self.num)
				self.numLives -= 1
			if incorrectOrder:
	...
	def run(self):
		self.num = 2
		while self.num <= self.matrixSize and self.numLives > 0:
			self.win.flip()
			self.reset()
